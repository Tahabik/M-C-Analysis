...
OUT_DIR="$HOME/perf-results/mem-profiles/tcg-dir"
mkdir -p "$OUT_DIR"
...
DATA_FILE_DEFAULT="$OUT_DIR/tcg_mem_default.data"
...
ssh -i ~/.ssh/id_lab -p 2223 root@localhost "./benchmarks/pc-mm" >"$WORKLOAD_OUT_DEFAULT" 2>&1
...
for size in "${sizes[@]}"; do
  for mode in "${modes[@]}"; do
    ...
    DATA_FILE="$OUT_DIR/tcg_mem_${kb_size}KB_${mode_label}.data"
    REPORT_FILE="$OUT_DIR/tcg_mem_report_${kb_size}KB_${mode_label}.txt"
    WORKLOAD_OUT="$OUT_DIR/tcg_workload_${kb_size}KB_${mode_label}.txt"
    ...
    ssh -i ~/.ssh/id_lab -p 2223 root@localhost "./benchmarks/pc-mm pointerchasing $size $mode" >"$WORKLOAD_OUT" 2>&1
